<template>
  <div id="app">
    <div class="header">
      <nav>
        <a href="depan.html">
            <img class="logo" src="./assets/S__17219904.jpg" alt="home"></a>
        <a class="atas" href="depan.html">Home</a>
        <a class="atas" href="konten.html">Content</a>
        <a class="atas" href="tentang.html">About Us</a>
        <div class="b_cari">
          <input class="search" type="text" placeholder="Cari..." required>
          <input class="search_button" type="button" value="Cari">
        </div>
        <div class="b_login">
          <input class="upload_button" type="button" value="UPLOAD">
        </div>
      </nav>
    </div>
    <router-view/>
    <div class="footer">
      <div>
        <a class="footer_a" href="https://www.instagram.com/bafe.id/" target="_blank">
          <img class="i_i" src="./assets/icon_instagram.png" style="padding-bottom: 0.5em">
        </a>
        <a class="footer_b" href="https://www.youtube.com/channel/UCj_AlXuthn7krLgr3Wxw3kA" target="_blank">
          <img class="i_y" src="./assets/icon_youtube.png">
        </a>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'App'
}
</script>

<style>
  a {
    color: #385446;
    text-decoration: none;
  }
#app {
  font-family: 'Open Sans', sans-serif;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
  text-align: center;
  color: #385446;
}

.atas {
  text-decoration: none;
  float: none;
  display: inline-block;
  border: 0px solid;
  padding: 40px 80px 0px 80px;
  font-size: 20px
}

.search {
  margin-left: 130px;
  margin-top: 30px;
  padding: 8px 45px 8px 8px;
  background: rgba(50, 50, 50, 0.2);
  border: 2px solid #385446;
  border-radius: 30px 0px 0px 30px;
}

.search_button {
  position: relative;
  padding: 8px 20px;
  left: -8px;
  border: 2px solid #385446;
  border-radius: 0px 30px 30px 0px;
  background-color: #385446;
  color: #fafafa;
}

.search_button:hover {
  background-color: #fafafa;
  color: #385446;
}

.upload_button {
  margin-left: 20px;
  margin-top: 20px;
  position: relative;
  padding: 20px 20px;
  border: 5px solid #385446;
  border-radius: 8px;
  background-color: #385446;
  color: #fafafa;
}

nav {
  display: flex;
}

.footer {
  position: relative;
  left: 0;
  bottom: 0;
  width: 100%;
  height: 60px;
  background-color: #385446;
  color: white;
  margin-top: 1%;
}
.footer_a {
  color: #fafafa;
  text-decoration: none;
  text-align: right;
}

.footer_b {
  color: #fafafa;
  text-decoration: none;
  text-align: right;
}

.i_y {
  height: 50px;
  width: 50px;
  float: right;
  padding: 5px 5px 0px 5px;
}

.i_i {
  height: 50px;
  width: 50px;
  float: right;
  padding: 5px 5px 0px 5px;
}
.logo {
    width: 100px;
    height: 100px;
    border: 0px;
    margin-right: 100px;
}
</style>
